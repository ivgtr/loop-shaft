...
describe('reward presentation', () => {
  ...
  it('persists only device presentation preferences and tolerates unavailable storage', () => {
    ...
    expect(loadPresentationSettings()).toEqual({ locale: detectLocale(), volume: .5, motion: false, highlights: false });
    savePresentationSettings({ locale: 'ja', volume: 0, motion: true, highlights: false });
    expect(loadPresentationSettings()).toEqual({ locale: 'ja', volume: 0, motion: true, highlights: false });
    stored = '{"volume":9,"motion":"yes"}';
    expect(loadPresentationSettings()).toEqual({ locale: detectLocale(), volume: 1, motion: false, highlights: false });

...
test('reward motifs share one presentation onset, keep cargo intact and respect effect controls', async ({ page }, info) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const firstImage = surface.capture();
    ...
    for (const [index, kind] of ['NATURAL_GOLD', 'GEM', 'FRACTURE_CORE', 'BLACK_GLASS_HEART'].entries()) {
      ...
      shots.push({ name: kind, image: surface.capture(), site: sitePixels() });
    }
    ...
    const quietImage = surface.capture();
  ...
  const action = (id: string) => page.locator(`[data-ui-action="${id}"]`);
  await action('help').click();
  await expect(action('presentation-volume')).toHaveAttribute('aria-label', /50/);
  await action('presentation-volume').click();
  await expect(action('presentation-volume')).toHaveAttribute('aria-label', /100/);
  await action('presentation-motion').click();
  await action('presentation-highlights').click();
  await page.reload();
  await action('help').click();
  await expect(action('presentation-motion')).toHaveAttribute('aria-label', /off/i);
  await expect(action('presentation-highlights')).toHaveAttribute('aria-label', /off/i);
});

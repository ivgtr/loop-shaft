...
import type { NoticeLine, WorldUi } from './worldUi';
...
export function drawShipmentNotice(ui: WorldUi, notice: ActiveRewardNotice,
  ...
  const text = (value: string, color = '#e1d8c3') => { lines.push({ text: value, color }); };
  if (highlight && age < payout) {
    const revealed = age >= APPRAISAL_TIMING.reveal;
    const graded = age >= APPRAISAL_TIMING.grade;
    const artwork = (ctx: CanvasRenderingContext2D, x: number, y: number) => {
      ctx.save(); ctx.translate(x - 92, y - 54);
    ...
    text(phrase('shipment.viaItems', { via, items: receipt.items }), '#a39887');
    text(label, revealed ? accent : '#b6a78e');
    text(graded ? (highlight.benefit ? displayText(locale, highlight.benefit) : valueText(highlight.detail, highlight.detailMessage))
      : phrase('shipment.surfaceAppraisal'), '#c8bfae');
    ui.notice(lines, 49, accent, artwork);
    return;
  }
  ...
  text(phrase('shipment.viaStatus', { via, items: receipt.items, status: phrase(total ? 'shipment.settled' : 'shipment.counting') }), '#a39887');
  text(phrase('shipment.scrapGain', { amount: displayed.toLocaleString('en-US') }), accent);
  ...
  text(total ? extra || phrase('shipment.credited') : special ? phrase('shipment.specialCargo', { amount: receipt.special })
    : phrase('shipment.ordinaryCargo', { amount: receipt.ordinary }), '#c8bfae');
  ui.notice(lines, 49, accent);
}
